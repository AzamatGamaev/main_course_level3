...
import static lesson6.Main.getArrayAfterLastNumber4;
...
class Method1 {
    ...
    void checkReturnArrayAfter3() {
        int[] expected = {0, 7, 2};
        int[] actual = getArrayAfterLastNumber4(sourceArr, 3);
    ...
    void checkReturnArrayAfter5() {
        ...
        int[] actual = getArrayAfterLastNumber4(sourceArr, 5);
        Assertions.assertArrayEquals(expected, actual);
    }
    ...
    void checkReturnArrayAfterLastNumber() {
        int[] expected = {};
        int[] actual = getArrayAfterLastNumber4(sourceArr, 2);
        Assertions.assertArrayEquals(expected, actual);
    }

    @Test
    void checkThrowExceptionWithAbsentNumber() {
        Assertions.assertThrows(RuntimeException.class, () -> getArrayAfterLastNumber4(sourceArr, 4));
    }

    @ParameterizedTest
    @MethodSource("multiData")
    void multiTest(int[] source, int number, int[] expected) {
        int[] actual = getArrayAfterLastNumber4(source, number);
        Assertions.assertArrayEquals(expected, actual);
    }
}
